...
return textChanged};
updateDimensions(){if(!this.dynamicSizing)return;
var isChangeBoxSize=this.isChangeBoxSize();
var isTextChanged=this.isTextChanged();
if(isChangeBoxSize||isTextChanged){this.saveProperties()}else{return};
...
updateDimensions2(){const oldWidth=this.width;
const oldLeft=this.left;
...
for(var i=0;i<textInfo.graphemeLines.length;i++){if(textInfo.graphemeText[charCount]==='\n'&&i>0){realLineCharCount=0;
charCount++;
...
return 1};
onInput(e){this.__inputType=e.inputType;
if(e.inputType!=='insertFromPaste'&&e.inputType!=='insertText'&&e.inputType!=='insertCompositionText'&&e.inputType!=='deleteContentBackward'&&e.inputType!=='deleteWordBackward'&&e.inputType!=='insertLineBreak'&&!(e.inputType==='deleteCompositionText'&&this.textSuggestBefore?.length===1)&&e.inputType!=='insertFromComposition'&&e.inputType!=='deleteByCut'){return};
if(e.inputType==='insertFromComposition'||e.inputType==='insertCompositionText'||e.inputType==='insertText'){this.__compositionData=e.data}else{this.__compositionData=''};
const fromPaste=this.fromPaste;
this.fromPaste=!1;
e&&e.stopPropagation();
if(!this.isEditing){return};
let nextText=this._splitTextIntoLines(this.hiddenTextarea.value).graphemeText,charCount=this._text.length,nextCharCount=nextText.length,removedText,insertedText,charDiff=nextCharCount-charCount,selectionStart=this.selectionStart,selectionEnd=this.selectionEnd,selection=selectionStart!==selectionEnd,copiedStyle,removeFrom,removeTo;
if(this.hiddenTextarea.value===''){this.styles={};
this.updateFromTextArea();
this.fire('changed');
if(this.canvas){this.canvas.fire('text:changed',{target:this,fromPaste});
this.canvas.requestRenderAll()};
return};
const oldRight=this.left+this.width;
if(e.inputType==='insertFromPaste'||this.hiddenTextarea.selectionStart!==this.hiddenTextarea.textLength){let textareaSelection=this.fromStringToGraphemeSelection(this.hiddenTextarea.selectionStart,this.hiddenTextarea.selectionEnd,this.hiddenTextarea.value);
let backDelete=selectionStart>textareaSelection.selectionStart;
if(selection){removedText=this._text.slice(selectionStart,selectionEnd);
charDiff+=selectionEnd-selectionStart}else if(nextCharCount<charCount){if(backDelete){removedText=this._text.slice(selectionEnd+charDiff,selectionEnd)}else{removedText=this._text.slice(selectionStart,selectionStart-charDiff)}};
insertedText=nextText.slice(textareaSelection.selectionEnd-charDiff,textareaSelection.selectionEnd);
if(removedText&&removedText.length){if(insertedText.length){copiedStyle=this.getSelectionStyles(selectionStart,selectionStart+1,!1);
copiedStyle=insertedText.map(function(){return copiedStyle[0]})};
if(selection){removeFrom=selectionStart;
removeTo=selectionEnd}else if(backDelete){removeFrom=selectionEnd-removedText.length;
removeTo=selectionEnd}else{removeFrom=selectionEnd;
removeTo=selectionEnd+removedText.length};
this.removeStyleFromTo(removeFrom,removeTo);
const markedLines=this.integrityCheck(insertedText);
if(markedLines.length>0&&selectionStart===this.text?.length){this.fixIntegrity(markedLines)}};
...
if(!copiedStyle){copiedStyle=[];
const baseStyle=this.getBaseStylesFromCursor();
for(let i=0,len=insertedText.length;i<len;i++){copiedStyle[i]={'fill':baseStyle.fill,'fontFamily':baseStyle.fontFamily,'fontSize':baseStyle.fontSize,'fontWeight':baseStyle.fontWeight,'fontStyle':baseStyle.fontStyle,'underline':baseStyle.underline,'overline':baseStyle.overline,'linethrough':baseStyle.linethrough,'deltaY':baseStyle.deltaY,'textBackgroundColor':baseStyle._grapheneColor||baseStyle.textBackgroundColor||'transparent'}}};
this.insertNewStyleBlock(insertedText,selectionStart,copiedStyle);
if(!fromPaste){const markedLines=this.integrityCheck(insertedText);
if(markedLines.length>0&&selectionStart===this.text?.length){this.fixIntegrity(markedLines)}}}};
this.updateFromTextArea();
this.fire('changed');
if(this.canvas){this.canvas.fire('text:changed',{target:this,fromPaste,inputEvent:e});
this.canvas.requestRenderAll()};
this.textSuggestBefore=e.data;
if(this.dynamicSizing){this.updateDimensions()};
this.setCoords()};
...
return this};
fixIntegrity(markedLines){for(let i=0;i<markedLines.length;i++){const markedLine=markedLines[i];
...
if(JSON.stringify(line[_key])==='{}'){line[_key]=Object.assign({},copiedStyle)}}}else{for(let i=0;i<len;i++){if(!copiedStyle)break;
line[`${i}`]=Object.assign({},copiedStyle)}};
keys=Object.keys(line)};
let styleObjs=[];
for(let j=0;j<keys.length;j++){const k=keys[j];
styleObjs.push(Object.assign({},line[k]))};
const newLine={};
let lastStyle=null;
for(let j=0;j<len;j++){if(j<styleObjs.length){newLine[`${j}`]=styleObjs[j];
...
for(let i=0,len=this._unwrappedTextLines.length;i<len&&lineKeys.length>0;i++){const key=lineKeys[i];
...
for(let i=0;i<keys.length;i++){const _key=keys[i];
...
this.isWrapping=!0;
for(i=0;i<lines.length;i++){var lineWrapped=this._wrapLine(lines[i],i,desiredWidth);
...
for(var i=0;i<words.length;i++){word=splitByGrapheme?words[i]:fabric.util.string.graphemeSplit(words[i]);
...
for(var i=0,len=word.length;i<len;i++){var box=this._getGraphemeBox(word[i],lineIndex,i+charOffset,prevGrapheme,skipLeft);
...
var lineHeights=0;
for(var i=0,len=this._textLines.length;i<len;i++){!this.__charBounds[i]&&this.measureLine(i);
...
ctx.restore()};
_renderRotatedChar(method,ctx,lineIndex,charIndex,left,top){!this.__charBounds[lineIndex]&&this.measureLine(lineIndex);
let charBox=this.__charBounds[lineIndex][charIndex],char=this._textLines[lineIndex][charIndex],localLineHeight=this.getHeightOfLine(lineIndex),drawLeft=left,drawTop=top+charBox.top+charBox.height;
const widthFactor=localLineHeight/this.lineHeight;
const heightFactor=charBox.width/2-charBox.height;
drawLeft=drawLeft-(widthFactor-charBox.width)/2;
drawTop=drawTop+heightFactor;
ctx.save();
const tx=drawLeft+charBox.width/2-charBox.height/8,ty=drawTop-charBox.height/8;
ctx.translate(tx,ty);
ctx.rotate(-Math.PI/2);
ctx.translate(-tx,-ty);
this._renderChar(method,ctx,lineIndex,charIndex,char,drawLeft,drawTop,0);
ctx.restore()};
_hasStyleChanged(prevStyle,thisStyle){if(!prevStyle&&thisStyle||prevStyle&&!thisStyle){return true};
if(!prevStyle&&!thisStyle){return false};
for(const prop in prevStyle){if(prevStyle[prop]!==thisStyle[prop]){return true}};
return false};
_renderChars(method,ctx,line,left,top,lineIndex){ctx.save();
left+=1;
for(var i=0,len=line.length-1;i<=len;i++){if(this._isRotatedChar(line[i])){this._renderRotatedChar(method,ctx,lineIndex,i,left,top)}else{this._renderCJKChar(method,ctx,lineIndex,i,left,top)}};
ctx.restore()};
_isRotatedChar(char){return ROTATED_BRACKETS_REGX.test(char)};
calcTextWidth(){let maxLineWidth=0;
for(let i=0,len=this.textLines.length;i<len;i++){const lineWidth=this.getHeightOfLine(i);
...
for(var lineIndex=0,len=this._textLines.length;lineIndex<len;lineIndex++){!this.__charBounds[lineIndex]&&this._measureLine(lineIndex);
currentLineHeight=0;
for(let charIndex=0,rlen=this._textLines[lineIndex].length;charIndex<rlen;charIndex++){char=this._textLines[lineIndex][charIndex];
charBox=this.__charBounds[lineIndex][charIndex];
if(char){if(this._isRotatedChar(char)){currentLineHeight+=charBox.width+space}else{currentLineHeight+=charBox.height+space}}};
...
for(var i=0,len=this._textLines.length;i<len;i++){if(width<=mouseOffset.x){lineHeight=this.getHeightOfLine(i)*this.scaleY;
width+=lineHeight;
lineIndex=i;
...
for(var j=0,jlen=line.length;j<jlen;j++){prevHeight=height;
charBox=this.__charBounds[lineIndex][j];
if(this._isRotatedChar(this._textLines[lineIndex][j])){height+=charBox.width*this.scaleY+space}else{height+=charBox.height*this.scaleY+space};
...
const isAlphaNumeric=this._isRotatedChar(this._textLines[lineIndex][charIndex-1]);
...
break};
for(var i=startLine;i<=endLine;i++){let lineHeight=this.getHeightOfLine(i),boxStart=0,boxEnd=0;
...
if(this._isRotatedChar(this._textLines[endLine][endChar-1])){boxEnd+=prevCharBox.width}else{boxEnd+=prevCharBox.height}}};
...
if(!this.__charBounds[i]||this.__charBounds[i].length===0){continue};
line=this._textLines[i];
boxHeight=0;
lastColor=this.getValueOfPropertyAt(i,0,'textBackgroundColor');
top=this.__charBounds[i][0].top;
for(var j=0,jlen=line.length;j<jlen;j++){char=line[j];
...
if(this._isRotatedChar(char)){boxHeight=charBox.width}else{boxHeight=charBox.height};
lastColor=currentColor;
top=charBox.top}else{if(this._isRotatedChar(char)){boxHeight+=charBox.kernedWidth}else{boxHeight+=charBox.height}}};
...
const underLineData=[];
let maxSize=0;
for(var j=0,jlen=line.length;j<jlen;j++){charBox=this.__charBounds[i][j];
char=line[j];
...
for(let k=1;k<underlinesData.length;k++){const data=underlinesData[k];
...
for(let t=0;t<=sectorIndex;t++){const sec=sectors[t];
if(sec&&sec.length>1){let maxX=sec[0].x;
let maxSize=sec[0].size;
let averageSize=0;
let numTotalChar=0;
let biasRatio=0;
for(let m=0;m<sec.length;m++){maxX=sec[m].x>maxX?sec[m].x:maxX;
...
for(let i=0;i<sec.length;i++){const _drawData=sec[i];
...
for(let i=1,len=this.textLines.length;i<len;i++){textWidth+=this.getHeightOfLine(i)};
return textWidth};
...
let textHeight=0;
for(let i=0,len=this.textLines.length;i<len;i++){const lineWidth=this.measureLine(i).width;
var rotatedHeight=0;
...
return baseStyle};
changeWidthAndHeight=wrapWithFireEvent('resizing',wrapWithFixedAnchor((_,transform,x,y)=>{const isTransformCentered=(transform)=>{return transform.originX==='center'&&transform.originY==='center'};
let target=transform.target,localPoint=getLocalPoint(transform,transform.originX,transform.originY,x,y),strokePadding=target.strokeWidth/(target.strokeUniform?target.scaleX:1),multiplier=isTransformCentered(transform)?2:1,changeWidth=!['mt','mb'].includes(transform.corner),changeHeight=!['ml','mr'].includes(transform.corner),oldWidth=target.width,oldHeight=target.height,newWidth=Math.abs(localPoint.x*multiplier/target.scaleX)-strokePadding,newHeight=Math.abs(localPoint.y*multiplier/target.scaleY)-strokePadding;
if(['tr','tl','bl','br'].includes(transform.corner)){const scaleFactor=Math.max(newWidth/oldWidth,newHeight/oldHeight);
